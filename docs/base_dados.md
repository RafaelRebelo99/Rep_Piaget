...
* **`materials`**: Centraliza os metadados dos ficheiros submetidos (PDF, DOCX, ZIP). Inclui o sistema de moderação através do campo `status` (`PENDING`, `APPROVED`, `REJECTED`).
...
1.  **Visibilidade:** Apenas utilizadores autenticados podem aceder a materiais com estado `APPROVED`.
2.  **Propriedade:** Os utilizadores têm permissão de edição ou eliminação estritamente sobre os seus próprios perfis e submissões.
...
* **`vw_course_structure`**: Consolida a hierarquia académica, expondo a relação curso-disciplina-semestre de forma direta.
