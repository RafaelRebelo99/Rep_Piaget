...
function isInstitutionalEmail(email: string) {
  return email.endsWith('@ipiaget.pt') || email.endsWith('@rep.pt')
}
...
export default function RecoverPasswordPage() {
  ...
  async function sendRecoveryEmail(targetEmail: string, resend = false) {
    const normalEmail = targetEmail.trim().toLowerCase()

    if (!isInstitutionalEmail(normalEmail)) {
      setSuccessMessage('')
  ...
  async function handleResendEmail() {
    ...
    await sendRecoveryEmail(submittedEmail || email, true)
  }
        ...
        <form noValidate onSubmit={handleSubmit} className="mt-8 space-y-5">
          ...
          {submittedEmail && (
            <button
              ...
              className="flex h-11 w-full items-center justify-center rounded-xl border border-[#87001f] bg-white text-sm font-bold text-[#87001f] transition hover:bg-[#f3e6ea] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {cooldown > 0
